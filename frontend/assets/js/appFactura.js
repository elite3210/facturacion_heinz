...
import { FacturaList } from "./classes/FacturaList.js"; // Clase refactorizada
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const facturaList = new FacturaList(
        ...
        'pagination',
        'pagination-info',
        (facturaId) => {facturaViewer.loadFactura(facturaId); }
    );
    facturaList.init();
...
function setupUploadForm(api, facturaList) {
    ...
    uploadForm.addEventListener('submit', async function (e) {
        ...
        try {
            ...
            // Enviar a la API
            const result = await api.uploadFactura(formData);

            // Mostrar resultado exitoso
            showUploadStatus('success', `Factura ${result.data.id} cargada correctamente`);
