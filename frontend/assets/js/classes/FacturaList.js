...
export class FacturaList {
    ...
    constructor(api, tableContainerId = 'factura-table-container',
        ...
        this.apiWrapper = {
            getData: async (filters, page, limit, sort, order) => {
                const result = await api.getFacturas(filters, page, limit, sort, order);
    ...
    generateCSV(facturas) {
        ...
        facturas.forEach(factura => {
            const row = [
                ...
                `"${factura.codigo_moneda || 0}"`,
                `"${factura.serie_numero_guia || ''}"`
            ];
    ...
    showPrintModal(selectedIds) {
        ...
        modal.querySelector('#confirm-print').addEventListener('click', () => {
            selectedIds.forEach((id, index) => {
                setTimeout(() => {
                    const url = `../../backend/api/facturaDownload.php?id=${id}&action=print`;
                    window.open(url, '_blank');
